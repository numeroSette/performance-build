...
func GetRandomNumberTest(b *testing.B) {
	...
	req, err := http.NewRequest("GET", "/get-random-number", nil)
	if err != nil {
		b.Fatal(err)
	}
	...
	handler := http.HandlerFunc(GetRandomNumber)
...
func BenchmarkGetRandomNumberTest(b *testing.B) { GetRandomNumberTest(b) }
